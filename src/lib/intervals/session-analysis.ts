...
function classifySessionType(
  ...
  const aiMap: Record<SessionType, string> = {
    INTERVAL_SESSION: [
      "PACE BREAKDOWN GUIDANCE: list each work interval individually — distance, pace, HR. Note pace drift and HR escalation rep by rep.",
      "For recovery laps: mention duration and whether HR dropped sufficiently before the next rep.",
      "Use work_intervals and recoveries from SESSION_INTERVAL_ANALYSIS_JSON as primary data.",
      "RUNNING MECHANICS GUIDANCE: if avg_stride_length_m is present across reps, describe the trend in coach language — e.g. 'Your stride shortened from 1.52m in rep 1 to 1.44m by the final rep, a sign of leg fatigue limiting push-off'.",
    ].join(" "),
    STEADY_RUN: [
      "PACE BREAKDOWN GUIDANCE: do NOT go lap-by-lap. Describe the pace arc of the whole run — how it opened, whether it held or drifted, and how it closed.",
      "Use single_effort_summary for the overall HR drift picture.",
      "RUNNING MECHANICS GUIDANCE: if single_effort_summary contains stride length drift data, describe whether mechanics remained consistent or softened over time.",
    ].join(" "),
    PROGRESSIVE_RUN: [
      "PACE BREAKDOWN GUIDANCE: describe how pace changed across the session. Was the acceleration controlled or rushed? Was it even across laps or did it come in surges?",
      "You may reference lap order for the pace trend but do not treat this as interval reps.",
      "RUNNING MECHANICS GUIDANCE: if stride length data is present, note how it adapted as pace increased — e.g. 'Stride length grew from 1.35m in the opening laps to 1.52m at race pace, a natural mechanical response to higher speed'.",
    ].join(" "),
    TEMPO_THRESHOLD: [
      "PACE BREAKDOWN GUIDANCE: describe the sustained effort block — opening pace, whether it held through the block, and any drift at the end.",
      "HEART RATE RESPONSE GUIDANCE: HR stability relative to pace is the key lactate threshold proxy here — focus on whether HR held steady or drifted despite consistent pacing.",
      "Do NOT analyse as short reps unless data clearly shows surges.",
    ].join(" "),
    MIXED_SESSION: [
      "PACE BREAKDOWN GUIDANCE: name each phase (warm-up, main set, cool-down) and describe the pace for each. If there is an embedded interval block, list each rep individually with distance and pace.",
      "Apply the right analytical lens per phase — aerobic for warm-up/cool-down, intensity-focused for the main set.",
    ].join(" "),
  };

  const embeddedIntervalInstructions = embeddedIntervalLike
    ? [
        "PACE BREAKDOWN GUIDANCE: this is a mixed session with an embedded interval block — do NOT flatten it into a single average pace.",
        "In the pace breakdown paragraph, name the three phases: warm-up (distance and pace), interval block (list each rep with its distance and pace), cool-down (distance and pace).",
        "For recovery laps between reps: note duration and whether HR dropped sufficiently before the next rep.",
        "RUNNING MECHANICS GUIDANCE: if avg_stride_length_m is present, comment on how mechanics changed from the aerobic warm-up into the interval block and back.",
      ].join(" ")
    : null;
